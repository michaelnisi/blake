...
vows.describe('Source').addBatch({
  ...
  'Source from input with header': {
    ...
    'without path': {
      topic: input.getSource('{ "template":"name.jade" }' + END, '/data/posts/path/to/file.md', paths),
    ...
    'with path': {
      topic: input.getSource('{ "template":"name.jade", "path":"/path/to" }' + END, 'path/to/file.md', paths),
      'should have the correct link': function(topic) {
        assert.strictEqual(topic.link, '/path/to/file');
      },
