...
// Resolve paths and copy resources from input to output.
var copyResources = function(input, output, callback) {
  var resolvedInput, resolvedOutput, reader, writer, pipe;

  resolvedInput = path.resolve(input);
  resolvedOutput = path.resolve(output);

	console.log('Copy resources from %s to %s', resolvedInput, resolvedOutput);

  reader = fstream.Reader({ path:resolvedInput });
  writer = fstream.Writer({ path:resolvedOutput, type:'Directory' });
  ...
  pipe.on('error', function(exception) {
    callback(exception);
  });
};
