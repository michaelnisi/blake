...
// Set default input and output pathnames. Create and return a paths object, 
// which contains all path definitions we need, especially the details of the
// input path are defined here.
var getPaths = function (config, inputPathName, outputPathName) {
  ...
  return {
    ...
    pathToResources: config.paths.resources 
                     ? path.join(inputPathName, config.paths.resources) 
                     : null,
    pathToData: config.paths.data 
                ? path.join(inputPathName, config.paths.data)
                : null,
    templatesPathName: config.paths.templates 
                       ? path.join(inputPathName, config.paths.templates)
                       : null,
    posts: config.paths.posts 
           ? path.join(inputPathName, config.paths.posts)
           : null,
...
var getSource = function (file, filename, paths) {
  ...
  header = JSON.parse(data.shift() || null);
  body = data.join(END_MARKER);
