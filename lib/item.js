...
module.exports = function (props, file, str) {
  var tokens = str.split(END_MARKER)
    , paths = props.paths
    , h = header(tokens, file, paths)
  if (!h) return
...
function header (tokens, file, paths) {
  var h = null
  try {
    h = JSON.parse(tokens.shift() || null)
  } catch (er) {
    throw(new Error('h required in ' + file))
    return h
  }
  if (!h.template) {
